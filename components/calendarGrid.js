...
const Day = styled.div`
  ...
  grid-template-rows: repeat(${(props) => props.numOfHours * 2}, 1fr);
`;
...
const events = [
  {
    ...
    start: dayjs().hour(3).minute(0).second(0),
    end: dayjs().hour(3).minute(30).second(0),
  },
];
    ...
    Days.push(
      ...
      >
        {events.map((event) => {
          ...
          let timeInterval = endTime.diff(startTime, "minute");
          console.log(startTime, endTime, timeInterval);
          return (
            <Event rowStart={1} rowEnd={timeInterval / 30 + 1}>
              <div></div>
            </Event>
          );
        })}
